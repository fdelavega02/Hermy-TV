...
async function ensureMemoryFiles(memoryCfg, root) {
  ...
  if (!existsSync(paths.longTerm)) {
    await writeFile(paths.longTerm, [
      ...
      '## Standing Notes',
      '',
      '- Hermy-TV is the stream-facing Twitch/OBS assistant, separate from private assistant memory.',
      '- Terminal Hermy and stream Hermy share this memory folder.',
      '',
      '## Memory Inbox',
      '',
    ].join('\n'));
...
export async function readSharedMemory(memoryCfg, root) {
  ...
  return [
    ...
    today ? `\nToday's raw notes:\n${today}` : '',
    '',
    'Use this as context. Do not quote private file paths or memory internals back to chat unless the streamer is explicitly asking about setup.',
  ].filter(Boolean).join('\n');
}
...
export async function appendSharedMemory(memoryCfg, root, entry) {
  ...
  const meta = entry.meta ? `\nMeta: ${clip(JSON.stringify(entry.meta), 800)}` : '';
